...
class StopWatchFragment : Fragment() {
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        val lastStarted = sharedPreferences?.getBoolean("Başladı", false)
        if (timeRunning != null && lastStarted == true) {
            if (timeRunning > 0) {
    ...
    private fun stopTimer() {
        val sharedPreferences = activity?.getSharedPreferences("stopWatchPref", MODE_PRIVATE)
        with(sharedPreferences?.edit()) {
            this?.putBoolean("Başladı", false)
            this?.apply()
        }
        context?.stopService(serviceIntent)
        binding.btnStart.text = "Başlat"
